...
**IMPORTANT**: From this lab onwards, the host machines for the instructor and
student computers now require internet access. In addition, it is important that
the host machine has the correct date and time configured.

**IMPORTANT:** Mention to students that the query language used in Log Analytics
is currently being updated across all regions in Azure. More information about
this can be found on the following website: <https://aka.ms/moc-10996A-m6-pg04>.
Explain to students that at the time of writing, the new query language is not
available in all regions. For this reason, queries used throughout this course
use the current query syntax. If, when performing the labs, they notice that
queries do not operate as expected due to an upgraded OMS workspace, they can
use the *Convert* option made available in the Log Search blade of the OMS
portal to convert the query to the updated syntax.
...
2.  Configure integration between OMS and System Center Operations Manager 2016

3.  Deploy and configure an OMS agent
...
####   Task 2: Configure integration between OMS and System Center Operations Manager 2016
...
-   Email or phone: **Your email address that is associated with the OMS
    Workspace**
...
####   Task 3: Deploy and configure an OMS agent
